...
**Status:** the bounded offline study is complete: 85 quality training runs plus six systems/pilot processes, 12 sealed checkpoints and eight frozen mixture policies evaluated on the 500,551-pair test, later-traffic evaluation, selected-checkpoint serving verification, and durable artifact verification. Sections 1–7 preserve the broader research agenda; executed work and explicitly deferred follow-ups are distinguished in the results.

## Abstract

We study how data composition, model capacity, supervision, feature availability and serving cost affect search relevance. The corpus contains 9.95M Gemini-labeled query–item pairs from natural exposure, query-balanced coverage and random unexposed 3D items. Across 85 quality runs, additional unique data helps, excessive repeated passes cause overconfidence, and random negatives improve rejection while trading off natural-traffic quality. On 500,551 query-disjoint held-out pairs, common-five accuracy is 73.04% for the full-data production-size feature model, 79.46% for a fully post-trained Qwen3-0.6B classifier on 250k query/title pairs, 81.33% for Qwen3-1.7B at matched data, and 82.04% for 0.6B on one million pairs. A frozen 1.7B/feature soft mixture reaches 82.09% and the lowest reported test NLL, 0.4746, versus 0.4798 for the compact 1M model. These recipe comparisons do not isolate representation, pretraining, capacity and compute. The matched classifier-versus-token contrast favors the classifier by 0.10 percentage points and 0.00122 NLL (Holm-adjusted p=0.0128 for NLL), a small practical effect. On 267 weighted complete pages from later traffic, the 1M model reaches 77.48% accuracy and 0.9347 page NDCG; a validation-calibrated 50% cascade routes 90.91% of future pairs to its LLM, exposing a budget-transfer failure. The selected 0.6B checkpoint passes a full-validation compilation gate while reducing warm batch-360 B200 latency from 147.96 to 51.59 ms. Statistical intervals, three-seed confirmations, feature-availability slices and recovery records accompany the results. These are teacher-agreement and standalone-system measurements; human preference, online effects and an end-to-end serving SLA remain unmeasured. Initial hypotheses and later registrations are retained in version history and are not retroactively presented as preregistered.
...
The network learns from scratch **on top of pretrained frozen embeddings**. Calling the entire system “trained from scratch” would hide the contribution and serving cost of those encoders. A raw-text random-initialized model is a separate baseline. Reported feature-model and hybrid parameter counts cover the reranking experts and assume existing embedding services; their full encoder footprint is not measured. The standalone 0.6B/1.7B text models include every parameter needed for their query/title scoring path and satisfy the sub-4B requirement directly.
...
| GPU allocation/budget | Two scheduler-reserved B200s verified; optimizer pilot passed on one; no total budget supplied; bounded stages completed |
...
## 10. Stage amendments and temporal evaluation progress

The 500k source-mixture screen uses 500 optimizer updates after the size screen exposed severe repeated-pass overconfidence at 2510 updates. The amendment preceded source-mixture outcomes. Six 2M follow-ups preserve 2510 updates and span natural-only, 75/25/0, 70/20/10, 50/25/25, 50/50/0, and 25/50/25 mixtures. Three-seed confirmation fixes the full-data production-size recipe and subset while changing initialization/minibatch seeds to 173, 271, and 811.
...
Numeric and query-embedding inputs come from the actual request logs. Item vectors are retrieved by explicit key from a fixed June 25 serving cache; only selected byte ranges are fetched, approximately 67 MB from two arrays totaling 20.67 GB, in addition to the 152 MB ID index. This predates both evaluation dates. Exact historical deployment identity is still unverified and will limit claims about end-to-end production replay. Current repository cache configuration refers to preprocessor version 3 under the version-9 package, whereas the available cache files are under preprocessor version 9; actual object discovery is recorded rather than trusting the stale constant.

The temporal teacher uses the repository's exact combined-request builder, combined prompt plus output format, Gemini 3.1 Pro preview alias, temperature zero, and the existing 3800-token cap. A 16-pair pilot succeeded before the full job. Every fetched thumbnail is saved by content hash and fetch time. Missing snapshot thumbnails are resolved from the current catalog; unavailable images are explicitly marked and use the prompt's title-based fallback policy. Therefore temporal visual agreement is not guaranteed to represent the original image at request time. A deterministic 128-pair reannotation bridge from the existing validation set estimates current-versus-original teacher disagreement; its findings accompany the future-traffic scores.
