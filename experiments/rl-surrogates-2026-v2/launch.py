...
def build_command(args):
    ...
    command = ["python", "/work/miles/train.py",
        ...
        "--use-dynamic-batch-size", "--max-tokens-per-gpu", str(args.packed_tokens),
        ...
        "--rollout-batch-size", str(args.prompts_per_rollout), "--n-samples-per-prompt", str(args.group_size),
        "--rollout-max-response-len", str(args.max_tokens), "--rollout-temperature", "1",
        "--rollout-top-p", "1", "--global-batch-size", str(args.global_batch_size),
        ...
        "--sglang-cuda-graph-max-bs", str(args.concurrency), "--sglang-max-running-requests", str(args.concurrency),
        ...
        "--n-samples-per-eval-prompt", str(args.eval_samples), "--eval-max-response-len", str(args.max_tokens),
        "--eval-temperature", "0", "--eval-top-p", "1",
        "--save", str(run_dir / "checkpoints"), "--save-interval", str(args.rollouts), "--no-save-optim",
    ...
    if args.no_checkpointing:
        command.remove('--gradient-checkpointing')
    if args.static_batch:
        command.remove('--use-dynamic-batch-size')
        i = command.index('--max-tokens-per-gpu'); del command[i:i+2]
    if args.no_save:
...
if __name__ == "__main__":
    ...
    p.add_argument("--algorithm", choices=["ppo", "dapo", "cispo", "glm5", "sapo", "dppo", "gspo"], default="ppo")
    p.add_argument("--dataset", choices=["gsm8k", "dapo"], default="gsm8k")
    p.add_argument("--split", default="train")
    p.add_argument("--rollouts", type=int, default=32)
    p.add_argument("--prompts-per-rollout", type=int, default=64)
    p.add_argument("--group-size", type=int, default=4)
    p.add_argument("--global-batch-size", type=int, default=16)
    p.add_argument("--packed-tokens", type=int, default=8192)
